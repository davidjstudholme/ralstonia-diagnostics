      ...
      workdir = ../05_workdir_renamed # directory where contigs/reads files are located and output is stored

    reference = 1  # 0:pick a random reference; 1:use given reference; 2: use ANI based reference
      reffile = GCA_029219985.1_ASM2921998v1_genomic.fna   # reference filename 

      project = Ralstonia_renamed_fasttree  # main alignment file name
